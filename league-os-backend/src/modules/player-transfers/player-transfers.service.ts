...
export class PlayerTransfersService {
  ...
  async findAll(currentUserId: number) {
    ...
    return this.transfersRepository.find({
      relations: { player: true, fromTeam: true, toTeam: true, createdByUser: true },
  ...
  async create(dto: CreatePlayerTransferDto, currentUserId: number) {
    ...
    if (dto.fromTeamId === dto.toTeamId) {
      throw new BadRequestException('Исходная и новая команды должны различаться');
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const [fromTeam, toTeam] = await Promise.all([
        teamRepository.findOne({ where: { id: dto.fromTeamId, isActive: true } }),
        teamRepository.findOne({ where: { id: dto.toTeamId, isActive: true } }),
      ]);
      if (!fromTeam || !toTeam) throw new NotFoundException('Команда не найдена');

      const source = await teamPlayerRepository.findOne({
        where: { playerId: dto.playerId, teamId: dto.fromTeamId, isActive: true },
        lock: { mode: 'pessimistic_write' },
      });
      if (!source) throw new NotFoundException('Игрок не состоит в исходной команде');
      ...
      if (activeLink?.id !== source.id) {
        throw new BadRequestException('Игрок уже состоит в другой активной команде');
      }

      if (dto.shirtNumber) {
        const duplicateNumber = await teamPlayerRepository.findOne({
          where: { teamId: dto.toTeamId, shirtNumber: dto.shirtNumber, isActive: true },
        });
        if (duplicateNumber) throw new BadRequestException('Номер уже занят в новой команде');
      }
      ...
      const futureMatches = await matchRepository.find({
        where: [
          { homeTeamId: dto.fromTeamId, status: MatchStatus.SCHEDULED, matchDatetime: MoreThan(new Date()) },
          { awayTeamId: dto.fromTeamId, status: MatchStatus.SCHEDULED, matchDatetime: MoreThan(new Date()) },
        ],
      });
      if (futureMatches.length) {
        const rosters = await rosterRepository
          .createQueryBuilder('roster')
          .where('roster.match_id IN (:...matchIds)', { matchIds: futureMatches.map((match) => match.id) })
        ...
        if (rosters.length) {
          await rosterPlayerRepository
            .createQueryBuilder()
            .delete()
            .where('match_roster_id IN (:...rosterIds)', { rosterIds: rosters.map((roster) => roster.id) })
      ...
      for (const oldStat of oldStats) {
        ...
        let newStat = await statsRepository.findOne({
          where: { tournamentId: oldStat.tournamentId, teamId: dto.toTeamId, playerId: dto.playerId },
        });
        if (!newStat) {
          newStat = statsRepository.create({
            ...
            isSuspended: oldStat.isSuspended,
            suspendedUntilMatchId: nextNewTeamMatch?.id,
            suspensionReason: oldStat.suspensionReason,
          });
        } else {
          newStat.yellowCards = Math.max(newStat.yellowCards, oldStat.yellowCards);
          newStat.redCards = Math.max(newStat.redCards, oldStat.redCards);
          newStat.secondYellowCards = Math.max(newStat.secondYellowCards, oldStat.secondYellowCards);
          newStat.isSuspended ||= oldStat.isSuspended;
          if (oldStat.isSuspended && nextNewTeamMatch) {
            newStat.suspendedUntilMatchId = nextNewTeamMatch.id;
          }
          newStat.suspensionReason ??= oldStat.suspensionReason;
        }
  ...
  private async ensureAdmin(userId: number): Promise<void> {
    ...
    if (!roles.includes(RoleCode.Admin) && !roles.includes(RoleCode.SuperAdmin)) {
      throw new ForbiddenException('Оформлять трансферы может только администратор');
    }
  }
}
